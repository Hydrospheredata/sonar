...
SEARCHGRPC = 'serving-grpc-scala'
REGISTRYURL = 'hydrosphere'
GITHUBREPO  = "github.com/Hydrospheredata/sonar.git"
...
def updateHelmChart(String newVersion){
  dir('helm'){
    //Change template
    sh script: "sed -i \"s/.*full:.*/  full: hydrosphere\\/${SERVICENAME}:$newVersion/g\" ${SERVICENAME}/values.yaml", label: "sed ${SERVICENAME} version"
    ...
    sh script: "frigate gen manager --no-credits > ${SERVICENAME}/README.md"

    //Lint manager
    dir('sonar'){
        sh script: "helm dep up", label: "Dependency update"
        sh script: "helm lint .", label: "Lint ${SERVICENAME} chart"

...
case class AcuracyMetricSpec(name: String, modelVersionId: Long, config: InputMetricSpecConfiguration, withHealth: Boolean = false, id: String = UUID.randomUUID().toString) extends MetricSpec
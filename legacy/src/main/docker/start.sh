...
echo ${JAVA} ${JAVA_OPTS} -cp "/app/app.jar:/app/lib/*" ${APP_OPTS} -Dlogback.configurationFile=logback.xml $1 # io.hydrosphere.sonar.legacy.Main
${JAVA} ${JAVA_OPTS} -cp "/app/app.jar:/app/lib/*" ${APP_OPTS} -Dlogback.configurationFile=logback.xml $1 # io.hydrosphere.sonar.legacy.Main

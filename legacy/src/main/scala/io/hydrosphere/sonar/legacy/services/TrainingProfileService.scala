...
import io.hydrosphere.sonar.common.terms.ProfileSourceKind
import io.hydrosphere.sonar.common.utils.{CollectionOps, CsvRowSizeMismatch, Logging, ProfileIsAlreadyProcessing}
import io.hydrosphere.sonar.common.utils.profiles.NumericalProfileUtils
import io.hydrosphere.sonar.common.utils.ContractOps._
import io.hydrosphere.sonar.common.utils.mongo.ObservableExt._
import io.hydrosphere.sonar.common.utils.CollectionOps
...
class TrainingProfileServiceInterpreter(config: Configuration, state: Ref[IO, Map[Long, TrainingProfileService.ProcessingStatus]], profileStorageService: ProfileStorageService[IO], mongoClient: MongoClient, autoODService: AutoODService[IO])(implicit cs: ContextShift[IO]) extends TrainingProfileService[IO, fs2.Stream] with Logging {
  ...
  def handleLocalFile(path: String, modelVersion: ModelVersion): Stream[IO, Byte] = {
    ...
    fs2.Stream.eval(s3CopyProgram) >>
      ...
      file.readAll[IO](Paths.get(path), cats.effect.Blocker.liftExecutionContext(ExecutionContext.fromExecutorService(Executors.newFixedThreadPool(2)), 2048))
  }
    ...
    processFile(path, modelVersion)
      ...
      .map(_.map(
        _.groupBy({
          ...
          .mapValues(_.toSeq.sortBy(_._1).map(_._2)).toMap)
      )
      ...
      .filter(_.nonEmpty)
      .chunkN(100)
      .flatMap((mapRows: Chunk[Map[String, Seq[String]]]) => {
